...
function Waterfall() {
    ...
    var onResize = self.__onResize = util.buffer(doResize, RESIZE_DURATION, self);
...
function timedChunk(items, process, context, callback) {
    ...
    function start() {
        ...
        todo = [].concat(util.makeArray(items));
...
function recalculate() {
    ...
    util.each(curColHeights, function (v, i) {
        curColHeights[i] = 0;
    });
    /**
...
util.extend(Waterfall, Base, {
    ...
    removeItem: function (item, cfg) {
        ...
        self.adjustItem(item, util.mix(cfg, {
            process: function () {
                item['remove']();
                return 0;
            },
